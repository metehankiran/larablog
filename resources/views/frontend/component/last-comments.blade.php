            ...
            <li class="wow fadeInUp animated @if(!$loop->last) mb-30 @endif">
                <div class="d-flex bg-white has-border p-25 hover-up transition-normal border-radius-5">
                    <div class="post-thumb post-thumb-64 d-flex mr-15 border-radius-5 img-hover-scale overflow-hidden">
                        <a class="color-white" href="{{ route('post',$comment->post).'#comment-id-'.$comment->id }}">
                            <img src="{{ Storage::url('public/images/default-user-profile-image.png') }}" alt="">
                        </a>
                    </div>
                    <div class="post-content media-body">
                        <p class="mb-10"><a href="{{ route('post',$comment->post).'#comment-id-'.$comment->id }}"><strong>{{ $comment->name }}</strong></a>
